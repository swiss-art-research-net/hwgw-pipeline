...
def prepareDataForMapping(*, module, inputFolder, outputFolder, filenamePrefix='item-', limit=None, offset=None, ids=None):
    ...
    if ids is not None:
        # id is filename, including the .xml suffix
    ...
    for file in files[offset:offset+limit]:
        if ids is not None or shouldBeMapped(file=file, metadata=metadata):
            ...
            prepareFileForMapping(file=file, inputFolder=inputFolder, outputFolder=outputFolder, preprocessor=preprocessor, module=module)
...
def prepareFileForMapping(*, file, inputFolder, outputFolder, preprocessor, module):
    with open(join(inputFolder, file), 'r') as f:
        with open(join(outputFolder, file), 'w') as g:
            contents = f.read()
            contents = preprocessor.preprocess(contents, module)
            g.write(contents)
